...
export class OpenAIEmbeddings extends Embeddings {
    ...
    async embedDocuments(texts) {
          ...
          try {
            ...
            if (error.message.includes("maximum context length")) {
              ...
              for (const text of batch) {
                try {
                   console.log("gomu gomu gomu");
                  await this.embeddingWithRetry({ ...params, input: [text] });
                  retryBatch.push(text);
                } catch (error) {
                  if (error.message.includes("maximum context length")) {
                    skippedDocuments.push(text);
                  } else {
                    throw error;
                  }
                }
              ...
              if (skippedDocuments.length > 0) {
                console.warn(
                  `Skipping ${skippedDocuments.length} document(s) that exceed the maximum token limit.`
                );
                skippedDocuments.forEach((text) => {
                  console.warn(`Skipped document: ${text.slice(0, 100)}...`);
                });
              }
      
              return this.embeddingWithRetry({ ...params, input: retryBatch });
            }
            throw error;
          }
        });
        ...
        for (let i = 0; i < batchResponses.length; i += 1) {
          ...
          embeddings.push(...batchResponse.map((response) => response.embedding));
        }
